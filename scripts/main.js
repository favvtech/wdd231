(function () {
  ...
  function renderCourses(list) {
    ...
    list.forEach(function (c) {
      ...
      card.className = 'course-card ' + (c.completed ? 'course-completed' : 'course-incomplete');
    ...
    if (creditsEl) {
      var total = list.reduce(function (sum, c) { return sum + (c.credits || 0); }, 0);
  ...
  function applyFilter(type) {
    ...
    if (type === 'WDD') data = data.filter(function (c) { return c.subject === 'WDD'; });
    else if (type === 'CSE') data = data.filter(function (c) { return c.subject === 'CSE'; });
    renderCourses(data);
  }
  ...
  if (filters && filters.forEach) {
    ...
    var defaultBtn = Array.prototype.find ? Array.prototype.find.call(filters, function (b) { return b.getAttribute('data-filter') === 'ALL'; }) : null;
